...
async def get_tools():
    mcp_config = utils.load_mcp_config("gmail", "yahoo-finance", "hackernews", "mlb")
...
async def stream_response(query, model_name, thread_id):
    ...
    model = ChatOpenAI(api_key=os.getenv("OPENROUTER_API_KEY"), base_url="https://openrouter.ai/api/v1", model=model_name)
...
@app.get("/")
async def read_root():
    return {"Hello Manjot Singh": "Your FastAPI server is up and running!"}

@app.post("/chat_stream")
async def chat_stream(request: ChatRequest):
    if not request.query.strip():
        raise HTTPException(status_code=400, detail="⚠️ Empty prompt!⚠️ ")

    try:
        return StreamingResponse(stream_response(request.query, request.model, request.thread_id), media_type="application")
    except Exception as e:
        raise HTTPException(status_code=500, detail="⚠️⚠️⚠️Internal Server crash!!!⚠️⚠️⚠️")
